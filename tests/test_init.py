...
def mock_env_setup():
    ...
    with patch("src.operations.init.Environment") as mock_env_class, \
...
@pytest.mark.parametrize("inputs, expected_context", [
    (
        [
            "zimbra", "/opt/zimbra/backup", "127.0.0.1", "secretpassword",
            "admin@example.com", "backup@example.com", 5, 15, "true", "SQLITE3"
        ],
        {
            'ose_user': "zimbra",
    ...
    (
        [
            "customuser", "/backup", "10.0.0.1", "pass",
            "user@test.com", "sender@test.com", 1, 7, "false", "TXT"
        ],
        {
            'ose_user': "customuser",
...
@pytest.mark.parametrize("exception_to_raise, expected_log", [
...
def test_run_init_template_load_failure(mock_geteuid, mock_env_setup):
    ...
    with patch("click.prompt", side_effect=["zimbra", "/tmp", "127.0.0.1", "p", "a@b.com", "a@b.com", 1, 1, "true", "TXT"]), \
...
def test_run_init_non_root():
    ...
    """
    with patch("src.operations.init.os.geteuid", return_value=1000), \
         patch("click.echo") as mock_echo:
