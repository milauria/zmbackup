...
class BackupSession(Base):
    ...
    session_name = Column(
        String(100),
        primary_key=True,
        comment="Unique session identifier: {type}-{timestamp}"
    )
    ...
    start = Column(
        DateTime,
        nullable=False,
        comment="Backup start timestamp"
    )
    ending = Column(
        DateTime,
        nullable=True,
    ...
    backup_type = Column(
        String(20),
        nullable=False,
        comment="Type: full, incremental, or mailbox"
    )
    description = Column(
        String(255),
        nullable=False,
        comment="Human-readable backup description"
    )
    ...
    size = Column(
        String(20),
        nullable=True,
        comment="Human-readable size (e.g., '76K', '1.2G')"
    )
    accounts_count = Column(
        Integer,
        nullable=True,
        comment="Number of accounts backed up"
    )
